...
EspWebManager::EspWebManager() : 
    server(80), shouldConnect(false), connectStatus(0), shouldTurnOffAP(false), turnOffTime(0), isTransferring(false) {}
...
void EspWebManager::setupRoutes() {
    ...
    server.on("/transfer-direct", HTTP_POST, [this](AsyncWebServerRequest *request){
        if (!request->hasParam("file")) {
            request->send(400, "text/plain", "Thiếu tên file");
            return;
        }

        if (this->isTransferring) {
            request->send(503, "text/plain", "Hệ thống đang bận truyền file khác");
            return;
        }

        String fileName = request->getParam("file")->value();
        this->transferFile = LittleFS.open("/" + fileName, FILE_READ);
        
        if (!this->transferFile) {
            request->send(404, "text/plain", "Không tìm thấy file");
            return;
        }

        // Bắt đầu quá trình truyền
        this->isTransferring = true;
        this->lastChunkTime = millis();
        
        // Gửi tín hiệu bắt tay
        Serial2.print("START:");
        Serial2.println(fileName);

        // Phản hồi về trình duyệt ngay lập tức
        request->send(200, "text/plain", "Bắt đầu truyền file");
    });
}
...
void EspWebManager::loop() {
    ...
    if (isConnecting) {
        if (WiFi.status() == WL_CONNECTED) {
            ...
            connectStatus = 1; 
            isConnecting = false;
        } else if (millis() - wifiStartTime > 15000) { // Timeout 15 giây
            connectStatus = -1;
            isConnecting = false;
        }
    }
    ...
    // Xử lý luồng truyền file không đồng bộ (Non-blocking UART transfer)
    if (isTransferring && transferFile) {
        // Mỗi 30ms gửi 1 chunk để Arduino Nano (9600 baud) kịp xử lý, tránh tràn bộ đệm
        if (millis() - lastChunkTime > 30) {
            if (transferFile.available()) {
                uint8_t buffer[64]; // Đọc từng khối 64 byte
                size_t len = transferFile.read(buffer, sizeof(buffer));
                ...
                Serial.println("Đã truyền xong file qua Serial2.");
            }
        }
    }
}
